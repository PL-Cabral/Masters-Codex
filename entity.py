...
## Criação da classe abstrata Entidade e seus paramêtros
@dataclass
class Entity(ABC):
    name: str
    max_hp: int
    ...
    def __post_init__(self):
        ...
        if self.max_hp <= 0:
            raise AttributeValidationError("O HP máximo precisa ser maior que zero.")

        if self.level <= 0:
            raise AttributeValidationError("O nível precisa ser maior que zero.")

        if self.current_hp is None:
            self.current_hp = self.max_hp

        if self.current_hp < 0:
            raise AttributeValidationError("O HP atual não pode ser menor que zero.")

        if self.current_hp > self.max_hp:
            raise AttributeValidationError("O HP atual não pode ser maior que o HP máximo.")
    ...
    def validate_can_act(self):
        if not self.is_alive():
            raise EntityCannotActError(f"{self.name} está com 0 de HP e não pode agir.")
    ...
    def add_skill(self, skill):
        ...
        for existing_skill in self.skills:
            ...
            if existing_name == skill_name:
                raise DuplicateSkillError(f"{self.name} já possui a habilidade '{skill_name}'.")
